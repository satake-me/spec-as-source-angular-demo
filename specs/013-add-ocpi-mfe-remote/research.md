...
## Decision 1: Reuse Native Federation manifest pattern from mf1/mf2

- **Decision**: Register `ocpi-mfe` in `public/federation.manifest.json` with `http://localhost:4203/remoteEntry.json` for development.
- **Rationale**: Host already resolves remotes (`mf1`, `mf2`) through the manifest; extending this pattern is the lowest-risk and most consistent path.
...
- **Decision**: Add an OCPI route in `app.routes.ts` using `loadRemoteModule(remoteName, './Component')`, mapped to the remote export, and fallback to `RemoteUnavailablePageComponent` on failure.
